...
const ChangelogEntrySkeleton = () => {
  return (
    <article className="scroll-mt-24 rounded-xl border border-neutral-200 bg-white p-6 dark:border-neutral-800 dark:bg-neutral-950">
      {/* Meta row */}
      ...
      <div className="mb-5 overflow-hidden rounded-xl border border-neutral-200 dark:border-neutral-800">
        <div className="p-4">
          ...
          <p className="mt-2 text-xs leading-relaxed text-neutral-500 dark:text-neutral-400">
            A running log of updates, fixes, and improvements to Issue Desk.
          </p>
        </div>
      </div>
...
const ChangeLog = () => {
  const { data: changelogs = [], isPending: loading } = useQuery<
    ChangelogData[]
  >({
    queryKey: ["GeneralChangelogsData"],
    queryFn: async () => {
      const response = await apiClient.get("/notifications/general-changelogs");
    ...
    >
      ...
      <div className="custom:px-8 mx-auto mb-8 max-w-6xl px-6 py-6 2xl:max-w-7xl">
        <div className="flex flex-col gap-12 lg:flex-row lg:gap-16">
          {/* Sidebar TOC */}
          <TableOfContents items={changelogs} isLoading={loading} />
          ...
          <div className="min-w-0 flex-1">
            ...
            {loading ? (
              <div className="space-y-6">
                {Array.from({ length: 8 }).map((_, index) => (
                  <ChangelogEntrySkeleton key={index} />
                ))}
              </div>
            ) : changelogs.length === 0 ? (
