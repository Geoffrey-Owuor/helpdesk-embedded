...
const DashboardLayoutShell = ({ children }: { children: React.ReactNode }) => {
  ...
  const defaultContent = (
    <HydrationGuard>
      ...
      <div
        id="main-content"
        className={`layout-scrollbar custom:right-2 custom:bottom-2 fixed top-14 right-0 bottom-0 left-0 scrollbar-gutter-stable overflow-y-auto rounded-t-3xl border border-[#eceef1] bg-white transition-all duration-200 ease-in-out sm:rounded-t-2xl sm:rounded-b-2xl ${showSidebar ? "custom:left-20" : "custom:left-2"} dark:border-neutral-900 dark:bg-black`}
      >
